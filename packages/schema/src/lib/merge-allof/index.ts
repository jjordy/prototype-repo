...
function merger(rootSchema, options, totalSchemas) {
  ...
  options.resolvers = Object.fromEntries(
    allResolverEntries.filter(([key, val]) => isFunction(val))
  );
  const complexResolvers = allResolverEntries.filter(([key, val]) =>
    isPlainObject(val)
  );
  ...
  function mergeSchemas(schemas, base, parents) {
    ...
    const complexKeysArr = complexResolvers.map(
      ([resolverKeyword, resolverConf]) =>
        allKeys.filter((k) =>
          [resolverKeyword, ...resolverConf.keywords].includes(k)
        )
    );
